...
export default {
	...
	onLoad(options) {
		...
		this.isShow=options.commentId==="undefined"?true:false;
	...
	methods: {
		submit() {
			this.isShow?this.$store.commit('setIsPoster',true):''
			var rule = [
				// 初步检查发布的内容
				{ name: 'title', checkType: 'string', checkRule: '3,50', errorMsg: '标题长度不符合规则。' },
				{ name: 'desc', checkType: 'string', checkRule: '3,300', errorMsg: '内容长度不符合规则。' }
			];
			...
			let finalFormInput = {};
			let imgUrl=this.oosArr.length<=0?[]:this.oosArr
			...
			// 	finalFormInput = Object.assign(this.formInput,{
			// 		isFather: true
			// 	},{commentImg:imgUrl,getUserInfo:getUserInfo})
			// 	
			// }
			 
			Parse.Cloud.run('postComment', finalFormInput)
				.then(r => {
				...
				// this.isShow?this.$store.commit('setLine',true):''
